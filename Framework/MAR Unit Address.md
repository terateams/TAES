...
> **MAR Unit Address** — 让每个业务单元同时拥有"位置"和"语境"，实现 AI 可理解的纯数字寻址
...
传统 URL 只解决"在哪里"（Where），不解决"是什么"（What）：

```
传统 URL：https://te3.lsn189.cn/team/0114
           └── 人能访问，但 AI 不理解这是"E队 suibe-org"
```

**核心矛盾**：
- 业务系统需要**稳定的地址**来导航
- AI Agent 需要**丰富的语境**来理解
- 两者如何**一一绑定**？

### 1.2 洞见：双地址绑定

> **核心创新**：每个业务单元同时拥有两个地址——**ua 指向位置，AC 指向语境**。
...
│  ══════════════════             ════════════════════════        │
│  U11.31.1.0114                  C31.1.0114                      │
│       │                              │                          │
│       ↓                              ↓                          │
│  位置：TAES/EdgeTeams/Team/0114      语境：CONTEXT/E队/0114      │
│  用途：导航、路由、API               用途：AI 理解、上下文注入   │
│                                                                 │
│  ───────────────── E=MC²AI 落地 ─────────────────────────────   │
│  ua 对应 C¹（Connection）    AC 对应 C²（Context）              │
│  两者绑定 = C² 的乘法效应                                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 地址 | 公式变量 | 问题域 | 指向 |
|:----:|:--------:|--------|------|
| **ua** | C¹ (Connection) | Where（在哪里） | Repo 内位置 |
| **AC** | C² (Context) | What（是什么） | CONTEXT/ 语境 |

### 1.3 方案：MAR 单元地址

> **MAR**（Mission Augmented Repo）= Repo 与 Mission 一一对应

```
Mission 的两层结构：

  Mission Framework（框架层）        Mission Instance（实例层）
  ════════════════════════          ═══════════════════════════
  定义方法论、协作模式                部署具体软件/站点
  形成 Mission Repo                  形成 App Repo
  例：TAES（U11）                    例：Te3.lsn189.cn（U12）
```

**地址格式**：
```
ua：U{MR}.{Seg}.{L2}.{ID}     例：U11.31.1.0114
AC：C{Seg}.{L2}.{ID}          例：C31.1.0114

转换：ua → AC = 去掉 U{MR}，前缀改 C
```

| 层级 | 定义 | 产出 | MR 示例 |
|------|------|------|---------|
| **Framework** | 方法论 + 协作模式 | Mission Repo | U11 = TAES |
| **Instance** | 具体问题的解决方案 | App Repo + 站点 | U12 = Te3.lsn189.cn |

### 1.4 设计目标

> **一句话**：类 TCP/IP 纯数字寻址 + AI 可理解的语境绑定

| 目标 | 说明 | 优先级 |
|------|------|:------:|
| **双地址绑定** | ua（位置）与 AC（语境）一一对应 | ⭐⭐⭐ |
