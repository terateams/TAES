...
| L1 菜单 | L2 子菜单 | 路由 | TAES地址 | 功能说明 | 实例地址示例 |
|---------|-----------|------|:--------:|----------|----------------|
| **TeamsCamp** | AITC | /Teamscamp/aitc | T3.2.1 | Allied Benefit Tenant 协同订阅 | T3.2.1.alliedai |
| | BAS | /Teamscamp/bas | T3.2.2 | Benefit Azure Subscription 赢力Azure | T3.2.2.sub-001 |
| | AGA | /Teamscamp/aga | T3.2.3 | Augmented Github Account 席位+算力 | T3.2.3.teamscamp |
| | Teamsbox | /Teamscamp/teamsbox | T3.2.4 | CPE 边缘设备（账单确权，专属） | T3.2.4.066217 |
| | Link Server | /Teamscamp/linkserver | T3.2.5 | VPN/Proxy 服务器池（订阅确权，共享） | T3.2.5.CN-SH-01 |
| | Service Node | /Teamscamp/servicenode | T3.2.6 | Apps Gateway 节点池（用量确权，共享） | T3.2.6.HK-RES |
...
> | 资源层 | L2 | E队 可用的 | 确权方式 |
> |:------:|:---|------------|----------|
> | **S** (Source) | Teamsbox (T3.2.4) | CPE 设备数、内置 S-L-N 路径方案 | 账单确权（专属） |
> | **L** (Link) | Link Server (T3.2.5) | VPN/Proxy 服务器数、地域分布、带宽容量 | 订阅确权（共享池） |
> | **N** (Node) | Service Node (T3.2.6) | Apps Gateway 节点数、IP 属性（住宅/DC/移动） | 用量确权（共享池） |
>
> - **路径矩阵**：网络能力 = S × L × N 的路径组合
> - **注**：【T】列出能力额度，【A】管技术运维
...
| 归属类型 | 主体 | 对象 | TAES地址 | 说明 |
|----------|------|------|:--------:|------|
| **持有** | T营 | AITC | T3.2.1 | 租户订阅 |
| **持有** | T营 | BAS | T3.2.2 | Azure 账户 |
| **持有** | T营 | AGA | T3.2.3 | GitHub 账号+席位+算力 |
| **持有** | T营 | Teamsbox | T3.2.4 | CPE 设备（账单确权） |
| **持有** | T营 | Link Server | T3.2.5 | 隧道服务器池 |
| **持有** | T营 | Service Node | T3.2.6 | 出口节点池 |
| **托付** | T营 → E队 | AO | T3.1.3 | 信任边界，运营授权 |
| **拥有** | E队 | AR + 交付物 | T3.1.4 | 免费创建，自由处置 |
