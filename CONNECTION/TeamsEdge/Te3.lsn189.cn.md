...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  L0: AC (Augment Context)  ← 团队的外化大脑（独立存在）         │
│       │                                                         │
│  L1: AR (Augmented Repo)   ← AC 的物理载体                      │
│       │                                                         │
│  L2: AO (Augmented Org)    ← AC 的归属边界                      │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  L3: TU (Task Unit)        ← AC 上发生的事件（瞬时）            │
│       │                                                         │
│       └── ca: "ca://..."   ← 只是指针，不是悬挂关系             │
│                                                                 │
│  主客关系：不是 TU 拥有 Context，而是 AC 容纳 TU                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  TU (Task Unit) — AC 上发生的事件                                │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  定义：在团队大脑（AC）上执行的最小业务单元                     │
│                                                                 │
│  属性：                                                         │
│  ├── tu_id: string        // 任务单元标识                       │
│  ├── ca: ContextAddress   // 语境地址（引用 AC，非内嵌）        │
│  ├── status: enum         // 状态                               │
│  └── payload: object      // 业务数据                           │
│                                                                 │
│  本体论定位：                                                   │
│  • TU 是 AC 上发生的「事件」，不是 Context 的「容器」           │
│  • AC 独立存在，TU 来来去去                                     │
│  • TU 完成后消亡，但 AC 记住了它（知识累积）                    │
│  • 多个 TU 可以引用同一个 AC（复用）                            │
│                                                                 │
│  用户体验：                                                     │
│  • 用户操作 TU，不需要理解 AC 结构                              │
...
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  AC (Augment Context) — 团队的外化大脑                   │   │
│  │  ═══════════════════════════════════════════════════════ │   │
│  │  • 独立存在，与团队同寿                                  │   │
│  │  • 存储于 AR (Repo) / CONTEXT/ 目录                      │   │
│  │  • 归属于 AO (Org) 的边界内                              │   │
│  │                                                         │   │
│  │       TU₁         TU₂         TU₃                       │   │
│  │        ↓           ↓           ↓                        │   │
...
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  本体论：AC 是「场」，TU 是「场上发生的事件」                   │
│  时间观：AC 恒久（大脑），TU 瞬时（念头）                       │
│  主客观：不是 TU 拥有 Context，而是 AC 容纳 TU                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 术语 | 全称 | 本体论定位 | 时间属性 | 类比 |
|:----:|------|------------|:--------:|------|
| **AC** | Augment Context | 团队外化大脑（主体） | 持久 | 大脑 |
| **TU** | Task Unit | AC 上的事件（发生的事） | 瞬时 | 念头 |
| **CA** | Context Address | 引用指针（非悬挂点） | — | 神经通路 |
| **CP** | Context Protocol | 寻址协议规范 | — | 神经协议 |
